-- Fix: calculate weekly km from actual runs (strava_awarded_runs) instead of stale cache
-- The cache (strava_stats_cache.weekly_total_km) doesn't auto-reset on Monday
...
BEGIN
  ...
  WITH weekly_posts AS (
    ...
    -- Calculate from actual runs this week, NOT from stale cache
...
-- Also fix the non-filtered version
...
BEGIN
  ...
  LEFT JOIN weekly_reactions wr ON wr.user_id = p.id
  LEFT JOIN weekly_strava ws ON ws.user_id = p.id
  WHERE COALESCE(wp.cnt, 0) + COALESCE(wr.cnt, 0) > 0 OR COALESCE(ws.km, 0) > 0
  ORDER BY COALESCE(ws.km, 0) DESC, COALESCE(wr.cnt, 0) DESC, COALESCE(wp.cnt, 0) DESC
  LIMIT p_limit;
END; $$;
