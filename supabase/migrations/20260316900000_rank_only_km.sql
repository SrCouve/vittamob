-- Rank da Semana: ONLY people who ran this week (km > 0)
-- Posts and energia don't count for appearing in the rank
...
BEGIN
  ...
  SELECT
    ...
    0::bigint AS weekly_energia,
    0::bigint AS post_count
  FROM profiles p
  JOIN (
    SELECT r.user_id, SUM(r.distance_km) AS km
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(ws.km, 0), 0::bigint, 0::bigint
  FROM profiles p
  JOIN (
    SELECT r.user_id, SUM(r.distance_km) AS km
    FROM strava_awarded_runs r
