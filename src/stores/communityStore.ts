...
export const useCommunityStore = create<CommunityState>((set, get) => ({
  ...
  fetchPosts: async (refresh = false) => {
    ...
    try {
      ...
      // Check which posts the current user has reacted to (batch query)
  ...
  fetchTopMembers: async () => {
    try {
      const { data, error } = await supabase.rpc('get_weekly_top_members', { p_limit: 5 });

      if (error) {
        // Fallback if RPC not deployed: return empty
        if (error.code === '42883') {
